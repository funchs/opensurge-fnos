...
# 说明装到的不是最新 fpk——请装 opensurge_0.1.2_*.fpk。
set -euo pipefail
...
if [ -z "$SRC64" ] || [ -z "$SRC256" ]; then
  ...
  echo "Reinstall opensurge_0.1.2_x86.fpk or _arm.fpk first."
  exit 1
fi
...
echo "   If still ~20KB/50KB → you installed an old fpk; install opensurge_0.1.2_*.fpk"
