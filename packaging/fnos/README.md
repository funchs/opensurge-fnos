...
版本号以 `fnos/manifest` 的 `version` 为准（当前 **0.1.3**），与镜像 tag
`v0.1.3` **保持一致**（`scripts/build.sh` 会把 compose 里的 `${VERSION}` 替换成同一数字）。
...
| 文件 | 说明 |
| --- | --- |
| `opensurge_0.1.3_x86.fpk` | Intel / AMD NAS |
| `opensurge_0.1.3_arm.fpk` | ARM64 NAS |
| `app.tgz` | 中间产物，被打进 fpk |

镜像（需另推）：`ghcr.io/funchs/opensurge-fnos:v0.1.3`  
`make docker-push-multiarch VERSION=v0.1.3`
