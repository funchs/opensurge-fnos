...
3. 安装过程中，按照系统提示输入这台 Mac 的管理员账户名称和登录密码——通常就是平时登录并可以安装软件的账户。
4. 安装完成后，从 `/Applications` 打开 **OpenSurge**。
...
3. When prompted, enter the name and login password of an administrator account on this Mac—normally the account used to sign in and install software.
4. After installation, open **OpenSurge** from `/Applications`.
