...
export function ConnectivityPage({ overview }: { overview: Overview | null }) {
  ...
  return <>
    ...
    {runtimeActive && <div className="notice actionable" role="status"><div><strong>Wi-Fi 重连后仍持续超时？</strong><p>可只重启 Mihomo 以重建 TUN 与出站 socket；不会停止 DHCP/DNS、卸载 nftables 规则或修改 NAS 网络设置，旧 Mihomo 日志会先归档。</p></div><button className="primary" type="button" disabled={recovering || testing.size > 0} onClick={() => void recoverMihomo()}>{recovering ? '正在恢复并复测…' : '仅重启 Mihomo'}</button></div>}
