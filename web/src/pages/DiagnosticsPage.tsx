...
import { api } from '../api'
import { formatTimestamp, PageHeader, SectionTitle, StatusDot } from '../components/Common'
...
export function DiagnosticsPage({ overview }: { overview: Overview | null }) {
  ...
  return <>
    <PageHeader eyebrow="DIAGNOSTICS" title="诊断、连接与 Provider" description="错误保持结构化；日志经过已知凭据脱敏。" />
    <section className="split"><div><SectionTitle title="Doctor" subtitle={overview?.doctor_healthy ? '基础检查通过' : '发现需要处理的问题'} />{overview?.doctor.map(check => <div className="check" key={check.name}><span className={check.ok ? 'ok-mark' : 'bad-mark'}>{check.ok ? '✓' : '!'}</span><div><strong>{check.name}</strong><small>{check.message}</small></div></div>)}</div><div><SectionTitle title="Proxy Providers" subtitle="可从这里观察和刷新" />{overview?.providers.proxy_providers.map(provider => <div className="row" key={provider.name}><StatusDot status={provider.proxies.some(proxy => proxy.alive) ? 'running' : 'degraded'} /><div className="grow"><strong>{provider.name}</strong><small>{provider.proxy_count} proxies · {provider.vehicle_type}</small></div><button onClick={() => void api.refreshProvider(provider.name)}>刷新</button></div>)}</div></section>
    <section className="section">
      <SectionTitle title="Live Connections" subtitle={details?.connection_error || `${details?.connections.connections.length ?? 0} active connections · 旁路由应出现 192.168.x sourceIP；若只有 198.18.x 则下游未进 TUN`} />
      <div className="inventory">
        <span>↑ {details?.connections.upload_total ?? 0} bytes</span>
        <span>↓ {details?.connections.download_total ?? 0} bytes</span>
        {details?.connections.connections.slice(0, 12).map(connection => {
          const meta = connection.metadata ?? {}
          const sourceIP = stringMeta(meta, 'sourceIP') || '—'
          const type = stringMeta(meta, 'type') || '—'
          const host = stringMeta(meta, 'host') || stringMeta(meta, 'destinationIP') || '—'
          const chain = (connection.chains ?? []).join(' → ') || connection.id.slice(0, 8)
          return <span key={connection.id} title={`${sourceIP} → ${host}`}>
            <code>{sourceIP}</code>
            {' · '}
            {type}
            {' · '}
