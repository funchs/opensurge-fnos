...
func (s *Store) SaveRecoveryCard(state RecoveryState) error {
	...
	card := fmt.Sprintf(`OpenSurge fnOS Edition - 同一 LAN DHCP 恢复卡
...
4. 回到 OpenSurge 执行 OFFER 探测，再将 NAS 网络接口恢复为自动 DHCP；接口 %q 的地址配置
   请在 fnOS 系统设置 → 网络中修改（OpenSurge 不改宿主机网卡，接口 %q 同上）。
5. 让客户端重新连接该 LAN（Wi-Fi 设备重连 Wi-Fi；有线设备重新获取地址），确认自动获取地址并能访问互联网。

重要：恢复自动获取的路径必须先确认路由器 DHCP 已恢复并通过 OFFER 探测，再把 NAS 切回自动 DHCP。
如果主动 OFFER 探测不可用，只能在人工确认路由器 DHCP 已恢复后，使用 Web GUI 中明确标注的
“跳过 OFFER 探测并恢复 NAS 自动 DHCP”兜底；该动作仍会真实恢复 NAS DHCP，不会只改状态标记。

如果你明确要让 NAS 长期保持当前静态 IPv4，可在 OpenSurge 网关停止后使用 Web GUI 的
“保留静态 IP 并结束”跳过路由器 DHCP 探测和 NAS 自动 DHCP 恢复。此选择不会验证或恢复
