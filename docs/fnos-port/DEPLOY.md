...
```
ghcr.io/funchs/opensurge-fnos:v0.1.3             # 推荐：固定版本，多架构
...
生产环境用 `v0.1.3` 这样的固定版本，别用 `latest`——它会随下次推送变化。
...
make docker-push-multiarch VERSION=v0.1.3   # 双架构 + 合并 manifest，并把 latest 指过去
```
...
`v0.1.3` 后重建容器。详见 [FPK-USER-GUIDE.md §8.1](./FPK-USER-GUIDE.md)。
