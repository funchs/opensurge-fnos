...
- **桌面入口**：`ui/config` 使用 `type: url`（浏览器打开，不做飞牛内嵌面板）。
...
**start 失败：`auto redirect: ... file exists`** —— mihomo `auto-redirect` 在部分内核
（常见 6.12+ nftables）上会 EEXIST。OpenSurge 启动 mihomo 时会设
`DISABLE_NFTABLES=true` 改走 iptables-legacy。若你仍看到该错误，请 pull 最新镜像；
临时也可在 config 里设 `tun_auto_redirect: false` 先让网关起来（旁路设备流量可能不进 TUN）。
